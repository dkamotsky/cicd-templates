...
class GenerationStepUnitTest(unittest.TestCase):
    ...
    def setUp(self):
        self.env = UnitTestEnvironment(spark=SparkSession.builder.master("local[1]").getOrCreate(), conf="generation_step_test.yaml")
    ...
    def test_sample(self):
        self.step()
        output_count = self.env.spark.read.format(self.step.output_format).load(self.step.output_path).count()
